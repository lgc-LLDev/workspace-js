...
function formResult(pl, query) {
  ...
  network.httpGet(
    `https://api.blackbe.xyz/openapi/v3/check/?name=${encodedQuery}&qq=${encodedQuery}&xuid=${encodedQuery}`,
    (code, ret) => {
      //if (code === 200) {
      ...
      if (retJson.success) {
        if (retJson.data.exist) {
          ...
          li.forEach((i) => {
            tmpLi.push("§r-=-=-=-=-=-=-=-=-=-=-=-=-=-", parseReturn(i));
          });
          content = tmpLi.join("\n");
        } else {
          content = `§a未查询到 §l§b${query} §r§a的记录§r：[§6${retJson.status}§r] §d${retJson.message}`;
        }
      } else {
        content = `§a查询失败§r：[§6${retJson.status}§r] §d${retJson.message}`;
      }
      //} else {
      //  content = `§4查询失败：请求失败，返回状态码${code}`;
      // }
      pl.sendForm(
        mc
          .newCustomForm()
...
function formQuery(pl) {
  pl.sendForm(
    mc
      ...
      .addLabel("§a请输入查询内容")
      .addLabel(
        "§6请谨慎使用XUID查询：由于历史遗留和XUID采集本身存在难度，导致大部分条目没有记录XUID，所以不推荐依赖XUID来判断玩家是否存在于黑名单"
      )
...
log("Load Successfully, Author: student_2333");
